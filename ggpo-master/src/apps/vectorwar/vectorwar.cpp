...
{
   renderer->SetStatusText("vw_advance_frame_callback");
...
{
   renderer->SetStatusText("vw_load_game_state_callback");
...
{
   renderer->SetStatusText("vw_save_game_state_callback");
...
{
   renderer->SetStatusText("vw_free_buffer");
 ...
 */
void
VectorWar_Init(HWND hwnd, int num_players, GGPOPlayer *players, int num_spectators)
{
 ...
 */
void
VectorWar_InitSpectator(HWND hwnd, int num_players, char *host_ip, unsigned short host_port)
{
   GGPOErrorCode result;
   renderer = new GDIRenderer(hwnd);
