    ...
    _In_ int)
{
   ...
   localport = (unsigned short)_wtoi(__wargv[offset++]);
   ...
   if (wcscmp(__wargv[offset], L"spectate") == 0) {
      ...
      VectorWar_InitSpectator(hwnd, num_players, host_ip, host_port);
   } else {
      ...
      VectorWar_Init(hwnd, num_players, players, num_spectators);
   }
   RunMainLoop(hwnd);
   VectorWar_Exit();
   WSACleanup();
